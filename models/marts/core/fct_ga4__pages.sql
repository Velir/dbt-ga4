...
with page_view as (
    select
        ...
        , concat(
            ...
            , cast(extract(hour from (select timestamp_micros(event_timestamp))) as string)
            , page_location
        ) as page_key
        ,-- would like to move this to dim_ga4__pages but need to think how to handle page_title changing over time        page_title
        ,
...
select
    join_conversions.* as except(page_key)
    , coalesce(scroll.scroll_events, 0) as scroll_events
from join_conversions
left join
    scroll on join_conversions.event_date_dt = scroll.event_date_dt and join_conversions.hour = scroll.hour and join_conversions.page_location = scroll.page_location and join_conversions.page_title = scroll.page_title
{% else %}
select
