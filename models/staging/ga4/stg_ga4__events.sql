...
include_event_key as (
    select 
        include_session_key.*,
        to_base64(md5(CONCAT(CAST(user_key as STRING), IFNULL(session_key, 'no_session'), event_name, CAST(event_timestamp as STRING)))) as event_key -- Surrogate key for unique events
